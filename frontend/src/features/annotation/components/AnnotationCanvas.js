...
import { SKELETON_CONNECTIONS, VISUAL_CONSTANTS, KEYPOINTS } from '../../../constants';
...
export const AnnotationCanvas = () => {
  ...
  // Use the video frame hook to handle video processing
  useVideoFrame();
  ...
  // Load frame image
  useEffect(() => {
    console.log('Frame image updated:', video.frameImage ? 'Image received' : 'No image');
  ...
  const renderKeypoints = useCallback(() => {
    ...
    annotation.persons.forEach(person => {
      ...
      Object.entries(personAnnotations).forEach(([keypointId, position]) => {
        ...
        const isSelected = annotation.selectedPersonId === person.id && 
    ...
    <div className="annotation-canvas">
      ...
      {annotation.selectedKeypoint && annotation.selectedPersonId && (
        <div className="annotation-hint">
          <span>
            Click to place <strong style={{ color: annotation.selectedKeypoint.color }}>
              {annotation.selectedKeypoint.name}
            </strong> for{' '}
            <strong style={{ color: annotation.persons.find(p => p.id === annotation.selectedPersonId)?.color }}>
              {annotation.persons.find(p => p.id === annotation.selectedPersonId)?.name}
            </strong>
          </span>
        </div>
      )}
